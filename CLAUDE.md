...
- 若目標位置已有不屬於本次計畫的檔案（`find_occupied_targets`），或產生的檔名去掉副檔名後為空（`find_empty_names`），同樣標記警告並停用執行；對調與連鎖的目標是計畫內來源，不算佔用

執行階段（`RenameService.execute_rename`）先驗證來源存在、來源未重複、新檔名未重複且不為空、目標未被計畫外的檔案佔用、讓位用的暫名未被佔用，任一不符即整批取消。
